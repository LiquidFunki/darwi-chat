...
size_t insert_data_stone(t_model_stone* model_stone) {
    ...
    char *insert_request = "INSERT INTO Stones(Id, Path) VALUES('%i', '%s');";
    char *sql_query = NULL;
    char *err_msg = NULL;

    asprintf(&sql_query, insert_request, model_stone->id, model_stone->path);
...
t_model_stone *get_stone_by_id(char *id) {
    ...
    if((err_status = sqlite3_exec(db, sql_query, callback_get_stone, model_stone, &err_msg)) != SQLITE_OK) {
        fprintf(stderr, "SQL_error: %s\n", err_msg);
        sqlite3_free(err_msg);
        sqlite3_close(db);
        return 0;
